...
module interrupts_tb;
	...
	// Scenario 0 (easy case): async interrupt right after a taken direct
	// branch. Fresh, non-overlapping slots so each PC keeps a unique
	// pcinfo type.
	//   0x1200, 0x1204 : warm-up linear (2 L)
	//   0x1208         : taken direct branch (BD) -> EASY_TGT
	//   0x1300         : branch target = async-interrupt resume PC (L)
	//   0x10f0         : EASY_ISR body (1 L) + 0x10f4 (R)
	//   0x1304         : inferable jump (JD) into the main scenarios
	localparam logic [31:0] EASY_BASE = 32'h0000_1200;
	localparam logic [31:0] EASY_TGT  = 32'h0000_1300;
	localparam logic [31:0] EASY_ISR  = 32'h0000_10f0;
	localparam logic [31:0] PROOF_ISR = 32'h0000_1110; // scenario 0b handler
	...
	initial begin
		...
		// ============================================================
		env.cpu.enter(.start_pc(EASY_BASE));

		// --- 0) EASY: async interrupt right after a taken DIRECT BRANCH ----
		//   A simpler instance of the post-control-flow async-interrupt case
		//   that scenario 8 reaches via an exception-return + tail-call. The
		//   last retired instruction is a taken direct branch (BD), so the
		//   interrupt's resume PC is the branch TARGET (0x1300), NOT the
		//   sequential next address. The encoder cannot derive that target
		//   from prior history alone (it is conveyed via tip.iaddr) — exactly
		//   the case the composer's interrupt source-PC handling and the
		//   async-INTERRUPT cross-check guard (riscv-trace-spec#324).
		env.cpu.run(8);                                              // 0x1200, 0x1204 (2 L)
		env.cpu.branch_taken(.target(EASY_TGT));                     // BD @0x1208 -> 0x1300 (taken)
		env.cpu.interrupt(.cause(7), .handler(EASY_ISR), .async(1)); // async @0x1300 — pc does not retire here
		env.cpu.run(4);                                              // EASY_ISR body: 0x10f0 (1 L)
		env.cpu.mret();                                              // R @0x10f4 -> resume AT 0x1300 (re-run)
		env.cpu.run(4);                                              // 0x1300 retires (L)

		// --- 0b) PROOF: the interrupt's own tip.iaddr is a DON'T-CARE -------
		//   An async interrupt after a SEQUENTIAL instruction. Here the prior
		//   instruction is not a taken CF, so pending_cf_next_iaddr is false and
		//   the composer does NOT read this beat's iaddr for any prior-branch
		//   target. We therefore drive a deliberately-WRONG tip.iaddr via
		//   .bad_iaddr(1) (=> 0xBAD0_BAD0). The encoder must ignore it entirely:
		//   the interrupt is encoded as ICNT + handler UADDR, and the post-ISR
		//   resume PC (0x130c) comes from the mret's target capture, not from
		//   this interrupt's iaddr. If the encoder used the poisoned iaddr the
		//   decode-pc check would FAIL — it passing is the proof.
		env.cpu.run(8);                                              // 0x1304, 0x1308 (2 L, sequential)
		env.cpu.interrupt(.cause(7), .handler(PROOF_ISR), .async(1), .bad_iaddr(1)); // async @0x130c, tip.iaddr POISONED
		env.cpu.run(4);                                              // PROOF_ISR body: 0x1110 (1 L)
		env.cpu.mret();                                              // R @0x1114 -> resume AT 0x130c (re-run)
		env.cpu.run(4);                                              // 0x130c retires (L)

		env.cpu.jump_to(.target(MAIN_PC));                           // JD @0x1310 -> enter main scenarios
		...
		env.cpu.exception_trap(.cause(ILLEGAL_INSTR), .handler(ISR_I)); // sync E @0x1044 -> ISR_I
