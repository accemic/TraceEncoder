...
/*
	task automatic compose_sync_msg(
		ref   nexus_message_t                     msg,
		input nexus_tcode_e	                      tcode,
		input nexus_sync_reason_e	              sync_reason,
		input nexus_icnt_t                        icnt,
		...
		clear_msg(msg,0);
		init_field(msg, 0, TCODE,    FIXED,     tcode,         $bits(nexus_tcode_e));
		init_field(msg, 1, SYNC,     FIXED,     sync_reason,   $bits(nexus_sync_reason_e));
		init_field(msg, 2, ICNT,     VARIABLE,  icnt,          LengthWoLeadingZeros(icnt));
		init_field(msg, 3, PC_FADDR, VARIABLE,  faddr,         LengthWoLeadingZeros(faddr));
		msg.id = msg.id+1;
	endtask
*/
	...
	//
	// These mirror the message shapes listed in
	// /home/albert/git/software/src/NexRv/NexRvMsg.h, producing a fully
