...
# P8 byte-neutrality gate: the explicit sync request over the TE register must
# not change a single byte of any existing stream. It is dormant until
# software writes trTeControl.InstSyncReq, so the claim is stronger than the
# usual one -- it holds with the feature compiled IN as well:
#
#   on  : the committed full profile (CT_EN_INST_SYNC_REQ = 1) must reproduce
#         the PINNED reference family bit for bit
#   off : the same build with the switch at 0 must do the same (zero cost
#         when off -- and the pre-P8 behaviour of the field)
#
...
# this gate inherited the anti-pattern -- P8 audit B-1). P8 adds no CAPS bit,
...
#
#   usage: bash scripts/p8_off_neutrality.sh on|off [worktree-path]
#
...
WT="${2:-${CT_WORKTREE_ROOT:-$(dirname "$repo")/ctte_worktrees}/p8_neutrality}"
REF="${REF:-}"          # empty => computed by pick_family() below
LOG="$repo/bld/p8_neutrality_${MODE}.log"
...
-- CT_EN_INST_SYNC_REQ carries no CAPS bit, so on and off select the same family"
	[ -f "$REF" ] || { say "FAIL: selected family file missing ($REF)"; exit 3; }
}

head="$(git rev-parse --short HEAD)"
say "source tree HEAD $head; worktree $WT; mode $MODE (CT_EN_INST_SYNC_REQ=$SW)"
...
sed -i -E "s/(localparam bit CT_EN_INST_SYNC_REQ[[:space:]]*=[[:space:]]*)[01];/\1${SW};/" "$WT/rtl/pkg/ct_pkg.sv"
...
out="$repo/bld/p8_manifest_${MODE}.txt"
: > "$out"
for d in $ALL_TBS; do
