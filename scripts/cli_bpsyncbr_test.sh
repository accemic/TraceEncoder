...
# Sync-boundary family: full compression suite x periodic sync WITHOUT an
# overflow (tests/overflow/08_bp_syncbranch). Found on hardware as
# "ICNT adjustment ERROR" and a short VendorBP walk.
# Gates:
#   B0  >=2 ProgTraceSync/IBHS after the start -> periodic axis active
...
if ct_prj_stale "$xd/${tb}_vlog.prj"; then
	...
	mkdir -p "$xd"
	sed "s|tests/overflow/01_overrun_recovery/${src_tb}.sv|tests/overflow/08_bp_syncbranch/${tb}.sv|" \
...
if [ "$syncs" -ge 2 ]; then echo "B0 PASS: periodic syncs active ($syncs)";
else echo "B0 FAIL: no periodic syncs"; fail=1; fi
if [ "$bps" -ge 1 ]; then echo "B1 PASS: BP path active ($bps VendorBP)";
else echo "B1 FAIL: no VendorBP message"; fail=1; fi
if grep -aq "Decoded OK" nexrv_suiteisync.log; then echo "B2 PASS: Decoded OK ($(grep -ac 'PC: 0x' nexrv_suiteisync.log) PCs)";
else
  echo "B2 FAIL (= Familie REPRODUZIERT): $(tail -3 nexrv_suiteisync.log | tr '\n' ' ')"
  fail=1
fi
