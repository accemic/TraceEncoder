...
# JTC vendor TCODE 57 OFF-vs-ON verification (local bring-up aid).
...
if ct_prj_stale "$xd/${tb}_vlog.prj"; then
	...
	mkdir -p "$xd"
	sed "s|tests/instruction/07_repeated_history/${src_tb}.sv|tests/instruction/09_jtc/${tb}.sv|" \
...
echo "### run ON ";  run_one on -testplusarg JTC
...
rb_on=$(grep -c 'VendorJTC' nexrv_on.log || true)
...
echo "VendorJTC      : $rb_on msgs in ON decode"
echo "------------------------------------------------------"
verdict=0
