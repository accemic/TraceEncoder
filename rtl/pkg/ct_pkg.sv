...
*
* @brief    CEDARtrace Package
*/

package ct_pkg;
	import ct_cs_cpuif_pkg::*;

//	localparam BRANCH_TRACE_MESSAGING		= 3; 			// BTM (Branch Trace Messaging) - every taken direct conditional branch is generating
//															// at least two byte message, but repeated branches may be counted and reported
//															// as a single message with a count (instead of many identical messages).
//	localparam BRANCH_HISTORY_MESSAGING		= 6; 			// HTM (Branch History Messaging) - every direct conditional branch (taken or not-taken)
//															// adds a single bit to the history buffer.
...
//	localparam	WALL_CLK_WIDTH				= 64;			// wallclock width
...
// 	localparam 	ATB_WIDTH					= 32;			// ATB bus width (bits)
