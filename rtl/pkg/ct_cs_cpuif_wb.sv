...
*
* @brief    CEDARtrace control / status registers
*           - distribution of control signals to the respective stages
*           - accessible by wishbone (via CPUIF)
*           - CDC for tip_clock
*           - CDC for proc_clk
*           - CDC for atb.aclk
*           - several signals don't require CDC, but are only accessible while trTeControl.Enable = 0
*/
...
/**
* @brief CDC helper: crosses a hwclr/hwset/value triple between a RDL hwif
*        clock domain and an external clock domain. hwclr/hwset are strobes
*        (ext -> hwif), value is a level (hwif -> ext).
*/
