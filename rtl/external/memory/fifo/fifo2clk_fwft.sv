...
module fifo2clk_fwft #(
	...
	// "auto": let the total storage decide (mirrors fifo1clk_fwft) so the
	// tool gets a block-RAM-capable template exactly when block RAM is the
	// appropriate structure.
	localparam STYLE =
		FIFO_STYLE != "auto"? FIFO_STYLE :
		(MIN_DEPTH*DATA_WIDTH >= 8192)? "block" : "distributed";
	localparam int unsigned  READOUT_LATENCY = (STYLE == "BLOCK") || (STYLE == "block")? 2+EXTRA_FABRIC_REGS : 1;
