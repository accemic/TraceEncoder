...
module ct_L2_mseo_mdo_formatter #(
	parameter int unsigned NEXUS_MAX_FIELDS           = nexus_vendor::NEXUS_MAX_FIELDS,           // max number of fields per nexus message
	parameter int unsigned NEXUS_MAX_FIELD_DATA_WIDTH = nexus_vendor::NEXUS_MAX_FIELD_DATA_WIDTH, // max payload width per field (bits)
	parameter int unsigned MDO_WIDTH,                                                             // number of MDO payload bits per chunk (no default — caller must set)
	parameter int unsigned ATB_CDC_FIFO_DEPTH         = 8                                         // min depth of proc->atb CDC fifo
) (
