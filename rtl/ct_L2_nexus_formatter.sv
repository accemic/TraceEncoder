...
*
* @brief    Nexus Msg formatter (@ proc_clk)
*			- Detailed knowledge of the Nexus protocol and the ATB protocol is required to understand how this module is working
*			- fields are explained here: https://github.com/riscv-non-isa/tg-nexus-trace/blob/master/docs/NexusTrace-TG-MessageDetails.adoc#71-fields-in-messages
*			- converts Trace Messages (SUB_MSG_CF, SUB_MSG_DF, SUB_MSG_DAQ) to Nexus Messages (without MDO/MSEO)
*			- a Nexus message consists of multiple fields
*			- a Nexus message always starts with a TCODE field
*
*
* @details
*   Converts C-Trace generic trace messages (CF/DF/DAQ) into a Nexus message
*   field list (`nexus_message_t`). This is the field-oriented representation
*   that the subsequent MDO/MSEO formatter packs into ATB chunks.
*
*   Backpressure:
		...
		if (proc_rst) begin
			...
			if (trace_msg.sub_type != SUB_MSG_NONE) begin
				...
				if (insert_src) begin
					...
					// 	NexusMsg.fields[IDX_DATA+0]	<= '{PROCESS,VENDOR_VARIABLE,		other._process, LengthWoLeadingZeros(trace_msg.other._process)};
					// end
//					NEXUS_MSG_VENDOR_CONFIG: begin
//						NexusMsg.fields[IDX_DATA+0]	<= '{INST_MODE,	VENDOR_VARIABLE, trace_msg.cfg.trTeInstMode, 	$size(ct_cs_cpuif__te__trTeControl__trTeInstMode_e_e)};		// VENDOR_FIXED causes a NexRV decoding //error (TBC...)
//					//	NexusMsg.fields[IDX_DATA+1]	<= '{SYNC_MODE,	VENDOR_VARIABLE, trace_msg.cfg.trTeInstSyncMode, $size(trTeInstSyncMode_e)};
//					//	NexusMsg.fields[IDX_DATA+2]	<= '{TS_ACTIVE,	VENDOR_VARIABLE, trace_msg.cfg.trTsActive, 		1};
//					//	NexusMsg.fields[IDX_DATA+3]	<= '{TS_TYPE,	VENDOR_VARIABLE, trace_msg.cfg.trTsType, 		$size(trTsType_e)};
//					end
