...
module ct_L1_funnel_tb;
	...
	task automatic test_flush_handling();
		...
		pulse_afready(0, 2);
		repeat (1) @(posedge atclk);
		void'(tc.tt_assert(dut.GlobalAckSeenQ[0] == 1'b1, "global ack sticky must set for channel 0"));
		void'(tc.tt_assert(global_flush_done == 1'b0, "global flush must still wait for remaining participants"));

		pulse_afready(1, 2);
		repeat (1) @(posedge atclk);
		void'(tc.tt_assert(dut.GlobalAckSeenQ[1] == 1'b1, "global ack sticky must set for channel 1"));
		void'(tc.tt_assert(global_flush_done == 1'b0, "global flush must still wait for empty participant"));

		pulse_afready(3, 2);
		repeat (1) @(posedge atclk);
		void'(tc.tt_assert(dut.GlobalAckSeenQ[3] == 1'b1, "global ack sticky must set for channel 3"));
