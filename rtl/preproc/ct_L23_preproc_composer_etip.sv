...
*
* @brief    CEDARtrace implementation (layer 2/3) eTIP composer
*           - composes control flow (cf), data flow (df), and data aquisition (daq) message
*           - parallel msg input, serial output with CDC to proc_clk
*
*           - later improvement: make etip_cvs_fifo width individual for cf, df and daq messages, not 3 x max(cf, df, daq)
*
*/
...
module ct_L23_preproc_composer_etip #(
	...
	parameter bit SPLIT_DATA_ACCESS = 0
) (
	...
	always_comb begin
		...
		ovf_inject_msg1.sub.cf.iaddr       = PrevIAddr;
