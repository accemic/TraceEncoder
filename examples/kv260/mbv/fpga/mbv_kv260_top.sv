 ...
 *   synchronizes pl_resetn0 into it.
 *
 *   Since D2 (2026-08-18) mbv_soc_top carries the shared three-sink subsystem
 *   ct_trace_sinks, so two more paths leave this top:
 *     DDR4 sink  mbv_soc_top's AXI4 write-only master -> PS S_AXI_HP0_FPD
 *                (saxigp2; the PS IP has had that port enabled all along --
 *                gen_ip.tcl PSU__USE__S_AXI_GP2 -- this top used to tie the
 *                inputs off, the duo/trio tops connected it)
 *     PIB        pib_clk/pib_data[3:0] to PMOD J2 (mbv_pib_pmod.xdc) -- the
 *                only external pins of this design; the PS pads are implicit.
 */

// A 1:1 copy of the tgc5b `ct_soc_kv260_top` of the predecessor repository,
// unchanged except for
// the module name and the SoC instance (`mbv_soc_top`). The four standalone
// IPs (ct_soc_kv260_ps/_rst/_dwc/_pc) keep their names -- gen_ip.tcl is
// reused verbatim.
//
// Migrated from an internal predecessor repository
// (2026-08-17).
module mbv_kv260_top (
	// PIB: parallel trace port on PMOD J2 (mbv_pib_pmod.xdc)
	output wire       pib_clk,
	...
	ct_soc_kv260_ps ps (
		...
		// Wired exactly like duo_kv260_top; before D2 these inputs were tied off
		// because this top had no DDR sink.
		.saxigp2_awid      (6'd0),
		...
		.saxigp2_rvalid    (),
		.saxigp2_rready    (1'b1),
		// S_AXI_HP1_FPD (saxigp3, present since the trio IP regen): silent --
		// only tie the handshakes (valid=0/ready=1), the rest may stay open.
