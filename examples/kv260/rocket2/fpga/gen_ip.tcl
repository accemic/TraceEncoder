...
# Sourced by run_rocket2_bitstream.tcl inside the open Vivado project
# context; each IP is created as a plain XCI with an explicit configuration
# and instantiated from rocket2_kv260_top.sv.
#
...
# AXI4 -> AXI4-Lite protocol converter -> rocket2_soc_top @ 0xA000_0000.
#
...
# byte-identical content to
# ../../rocket_linux/fpga/gen_ip.tcl (both S_AXI_GP2 and S_AXI_GP3 needed by
# this example too: the trace DDR sink and the Rocket memory path). Vendored
# per-example rather than cross-referenced, same reasoning as mbv's own copy
# and as rocket_linux's: no cross-example TCL dependency for a small,
# self-contained generation script.
...
# (ct_soc_ddr_sink AXI master) -- write-only, see rocket2_kv260_top.sv.
...
# memory map). ONE port shared by both harts -- the generat bus is shared,
# TLBroadcast keeps the L1s coherent (M2 gate (e)).
...
# at that, the constant-high tie in rocket2_kv260_top.sv would hold
# everything in permanent reset and every PS access would hang).
