...
WHY: run_rocket2_bitstream.tcl adds the CTTE encoder sources with plain
`add_files` (it predates this repo's `abc` build driver, see the mbv
example's README "abc flow" note) -- so it needs a file list, and this reads
that list out of the encoder's own dependency graph (`.abc`) instead of
hand-maintaining one that would silently drift on the next change.
