...
/**
 * @brief    Bitstream top of the Rocket RV64 demonstrator, two-hart (M4).
 *
 * @details
 *   Identical PS provisioning as cva6_linux_kv260_top / trio_kv260_top --
 *   the same four XCIs from gen_ip.tcl, the same 75 MHz domain. Only the SoC
 *   instance differs. Three PS interfaces:
 *
 *     M_AXI_HPM0_FPD (128 bit) -> dwc 128->32 -> pc AXI4->Lite
 *         -> rocket2_soc_top @ 0xA000_0000 (4 MiB aperture)
 *     S_AXI_HP0_FPD (saxigp2, 32 bit)  <- trace DDR sink (write-only)
 *     S_AXI_HP1_FPD (saxigp3, 64 bit)  <- Rocket memory path (read+write)
 *
 *   Memory map (examples/kv260/SPEC_board_memory_map.md, reserved-memory
 *   ctrace_resmem.dtso). ADDRESS PLAN v4 since 2026-08-10 -- this header
 *   carried the v3 numbers until 2026-08-21 while the RTL had long since
 *   reset to v4 (rocket2_soc_top.sv, `ddr_base_reg`/`ddr_size_reg`). A
 *   header that contradicts its own reset values is worse than none:
 *     0x5000_0000 + 256 MiB  trace-sink ring (v3 was 0x6000_0000 + 64 MiB;
 *                            64 MiB wraps every 2.3 s at 29.7 MB/s, 256 MiB
 *                            every 9.0 s -- that is why it moved)
 *     0x6000_0000 + 64 MiB   GAP (the former window; before v4 sink and
 *                            guest abutted with zero bytes of margin)
 *     0x6400_0000 + 192 MiB  Rocket Linux RAM (OpenSBI payload @ 0x6400_0000)
 *
 *   THE DIFFERENCE TO THE CVA6 DESIGN that matters here: the addresses do
 *   NOT pass through 1:1. The Rocket bus only decodes memory starting at
 *   0x8000_0000 (fixed in the generat, L2 decision D-L2-1), so the guest
 *   sees 0x8000_0000 and the window guard inside the wrapper translates to
 *   0x6400_0000. What lies outside the window gets DECERR instead of
 *   silently reaching into foreign PS DDR.
 *
 *   TWO-HART VERSION (M4). What changes relative to rocket_kv260_top changes
 *   EXCLUSIVELY below this file -- the PS provisioning, the three
 *   interfaces, the memory map, and the aperture are unchanged. Only the SoC
 *   instance is swapped:
 *
 *     rocket_soc_top  -> rocket2_soc_top   (generat rocket64t2, two harts,
 *                                           two ct_encoder, ct_L1_funnel,
 *                                           two ENC CSR windows)
 *
 *   EN_ETRACE is therefore **0** by default, not 1: the funnel recognizes
 *   packet boundaries via the Nexus MSEO bits, an E-Trace backend delivers
 *   raw bytes without MSEO. rocket2_soc_synth_wrap aborts elaboration at
 *   EN_ETRACE = 1 instead of silently merging wrong. (For the one-hart
 *   branch, 1 was correct -- there is no funnel there.) Independent of that,
 *   CT_XLEN = 64 already requires the value 0, because the te_inst address
 *   fields would otherwise truncate (R1.1 D-7).
 *
 *   The trace path is ONE stream: both encoders feed into the funnel, whose
 *   output runs into the same 1-MiB URAM ring as the one-hart design. The
 *   sources are only split apart at the decoder, on the Nexus SRC field
 *   that each instance carries via CSR.
 */
module rocket2_kv260_top #(
	// E-Trace backend. Default 1 = the R4a stand (DUAL, protocol choice via
	// CSR). With an encoder at ct_pkg::CT_XLEN = 64 it MUST be 0: ct_encoder
	// rejects the combination at elaboration with $fatal, because the
	// te_inst address fields hang off CT_ETRACE_ADDR_BITS = 31 and would
	// silently truncate (X2b not implemented, R1.1 D-7). The value comes
	// from run_rocket_bitstream.tcl (first tclarg) -- before package W1
	// this tclarg was read but never wired up anywhere (B-W1-2).
	parameter bit EN_ETRACE = 1'b0
) (
	// PIB: parallel trace port on PMOD J2 (rocket_pib_pmod.xdc)
	output wire       pib_clk,
	...
	// -- Rocket memory path: SoC's AXI4 master -> PS S_AXI_HP1_FPD (saxigp3) --
	...
	uwire logic [5:0]  hp1_bid_w, hp1_rid_w;   // 6 bit on the PS side
	...
	ct_soc_kv260_ps ps (
		...
		.saxigp2_rvalid    (),
		.saxigp2_rready    (1'b1),
		// S_AXI_HP1_FPD (64 bit) <- Rocket memory path.
		// No atop input on the HP port -- none needed either: the Rocket
		// resolves the A-extension internally (L1D/TLAtomicAutomata), the
		// window guard drives m_awatop constant 0
		// (rocket_mem_window.sv:210).
	...
	rocket2_soc_top #(
		.WIN_BASE    (64'h8000_0000),   // guest view, fixed in the generat (D-L2-1)
		.WIN_SIZE    (64'h0C00_0000),   // 192 MiB == memory node in the DTS
		.PS_BASE     (64'h6400_0000),   // reserved PL DDR window (SPEC v3)
		.UART_BASE   (64'h6001_0000),   // MMIO window of the Rocket bus
		.CON_BYTES   (65536),
		.TRACE_DEPTH (262144),          // 1 MiB URAM ring (like CVA6/Trio)
