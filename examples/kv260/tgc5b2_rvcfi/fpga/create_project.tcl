...
# bitstream top tgc5b2_rvcfi_kv260_top + the 5 standalone IPs (gen_ip.tcl).
#
# The run ends with an elaboration gate: `synth_design -rtl -top
# tgc5b2_rvcfi_soc_top` checks every RTL building block without waiting for
# the PS IP generation; it also prints the FIFO IP's instantiation template
# (port-contract verification for tgc5b2_rvcfi_kv260_top). Full bitstream build:
# run_bitstream.tcl.

set script_dir [file dirname [file normalize [info script]]]
set repo_root  [file normalize [file join $script_dir .. .. .. ..]]
set proj_name  tgc5b2_rvcfi
...
set board_files [list \
	...
	[file join $script_dir .. rtl ct_tip_adapter_actcap.sv] \
	[file join $script_dir .. rtl ct_soc_shared_mem.sv] \
	[file join $script_dir .. rtl ct_soc_doorbell.sv] \
	[file join $script_dir .. rtl ct_soc_console.sv] \
	[file join $script_dir .. rtl ct_axil_demux2.sv] \
	[file join $script_dir .. rtl tgc5b_rvcfi_synth_wrap.sv] \
	[file join $script_dir .. rtl tgc5b2_rvcfi_soc_top.sv] \
	[file join $script_dir tgc5b2_rvcfi_kv260_top.sv] \
]
...
puts "### ELAB-CHECK: synth_design -rtl -top tgc5b2_rvcfi_soc_top"
if {[catch {synth_design -rtl -top tgc5b2_rvcfi_soc_top -mode out_of_context} elab_err]} {
	puts "### ELAB_FAIL: $elab_err"
	exit 2
}
puts "### ELAB_OK (tgc5b2_rvcfi_soc_top elaborates)"
close_design
