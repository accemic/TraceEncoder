 ...
 *   (b) Module renamed ct_soc_synth_wrap → tgc5b_rvcfi_synth_wrap (both the
 ...
 *       its word-3 metadata.
 *
 *   TWO FURTHER DEVIATIONS FOR THE RV/CFI DEMO (this copy):
 *
 *   (d) NEW external dBus segment port `ext_*` (AXI4-Lite master). The dBus
 *       decoder gains a fourth segment for address nibbles 3 and 4, which the
 *       reference maps into its `default -> RAM` arm. Everything in that
 *       segment leaves the wrapper on `ext_*` and the SoC top decides what it
 *       is:
 *           0x3000_0000  SHARED    the dual-port shared memory (both cores)
 *           0x4000_0000  ACTCAP    the ACT-CAP doorbell (private per core)
 *       Note what this changes for a typo: in the reference, a store to
 *       0x3000_0000 lands SILENTLY in RAM, because everything unrecognised
 *       aliases there. Here it goes out of the wrapper and the top answers
 *       it, so a mistyped pointer produces a visible access instead of quiet
 *       memory corruption.
 *
 *   (e) `ct_tip_adapter` replaced by `ct_tip_adapter_actcap`, which turns a
 *       store to the doorbell address into the CSR-write TIP beat the
 *       encoder's ACT-CAP stage decodes. The TGC5B accepts CSR writes to the
 *       vendor window 0xB10-0xB9F without trapping but never reports them on
 *       its trace port, so software instrumentation is otherwise unreachable
 *       on this core. See that module's header for the full reasoning; set
 *       `EN_ACTCAP = 0` to get the reference adapter's behaviour back.
 *
 *   Everything else (core, encoder, RAM + loader mux, CLINT/INTC, dBus
 *   decode for segments 0-2, external Wishbone config port) is unchanged
 *   from the reference — see that file's header for the memory map and the
 *   core_rst_hold / cfg_wb_en contracts.
 */

module tgc5b_rvcfi_synth_wrap #(
	...
	// Deviation (e): ACT-CAP doorbell address, and the switch that turns the
	// synthesis of the CSR beat off (for a core that reports CSR writes
	// itself, or to measure the demo without software instrumentation).
	...
	// -- External dBus segment (deviation (d)): AXI4-Lite MASTER -----------
	// Everything the core addresses in nibbles 3 and 4 leaves here; the SoC
	// top decodes SHARED (0x3xxx_xxxx) vs ACTCAP doorbell (0x4xxx_xxxx) from
	// the address it gets.
	output      logic         ext_awvalid,
	input  uwire logic        ext_awready,
	output      logic [31:0]  ext_awaddr,
	output      logic         ext_wvalid,
	input  uwire logic        ext_wready,
	output      logic [31:0]  ext_wdata,
	output      logic [3:0]   ext_wstrb,
	input  uwire logic        ext_bvalid,
	output      logic         ext_bready,
	input  uwire logic [1:0]  ext_bresp,
	output      logic         ext_arvalid,
	input  uwire logic        ext_arready,
	output      logic [31:0]  ext_araddr,
	input  uwire logic        ext_rvalid,
	output      logic         ext_rready,
	input  uwire logic [31:0] ext_rdata,
	input  uwire logic [1:0]  ext_rresp,

	// -- ACT-CAP observation (deviation (e)) -------------------------------
	output      logic [31:0]  actcap_count,
	...
	// Deviation (e): the ACT-CAP-aware adapter. Same mapping as the reference
	// plus the doorbell-store -> CSR-write rewrite.
	ct_tip_adapter_actcap #(
		.DOORBELL_ADDR (DOORBELL_ADDR),
		.EN_DOORBELL   (EN_ACTCAP)
	) tip_adapter_inst (
		.clk                (clk),
		.rst                (rst),
	...
	always_comb begin
		...
		ext_awvalid    = 1'b0; ext_wvalid    = 1'b0;
		if (!b_pending) begin
			unique case (wr_seg)
				...
				SEG_EXT:    begin ext_awvalid    = dbus_awvalid; ext_wvalid    = dbus_wvalid; end
				default:    begin ram_awvalid    = dbus_awvalid; ram_wvalid    = dbus_wvalid; end
			endcase
		end
	end

	assign ext_awaddr = dbus_awaddr;
	assign ext_wdata  = dbus_wdata;
	assign ext_wstrb  = dbus_wstrb;
	assign ext_araddr = dbus_araddr;

	assign dbus_awready = !b_pending &&
		((wr_seg == SEG_PERIPH) ? periph_awready :
		 (wr_seg == SEG_ENC)    ? enc_awready    :
		 (wr_seg == SEG_EXT)    ? ext_awready    : ram_awready);
	assign dbus_wready  = !b_pending &&
		((wr_seg == SEG_PERIPH) ? periph_wready  :
		 (wr_seg == SEG_ENC)    ? enc_wready     :
		 (wr_seg == SEG_EXT)    ? ext_wready     : ram_wready);

	// Write response mux (by latched b_seg).
	assign dbus_bvalid =
		(b_seg == SEG_PERIPH) ? periph_bvalid :
		(b_seg == SEG_ENC)    ? enc_bvalid    :
		(b_seg == SEG_EXT)    ? ext_bvalid    : ram_bvalid;
	assign dbus_bresp  =
		(b_seg == SEG_PERIPH) ? periph_bresp  :
		(b_seg == SEG_ENC)    ? enc_bresp     :
		(b_seg == SEG_EXT)    ? ext_bresp     : ram_bresp;
	...
	assign ext_bready    = (b_seg == SEG_EXT)    && dbus_bready;
	...
	always_comb begin
		...
		if (!r_pending) begin
			unique case (rd_seg)
				...
				SEG_EXT:    ext_arvalid    = dbus_arvalid;
	...
	assign dbus_arready = !r_pending &&
		((rd_seg == SEG_PERIPH) ? periph_arready :
		 (rd_seg == SEG_ENC)    ? enc_arready    :
		 (rd_seg == SEG_EXT)    ? ext_arready    : ram_arready);

	// Read response mux (by latched r_seg).
	assign dbus_rvalid =
		(r_seg == SEG_PERIPH) ? periph_rvalid :
		(r_seg == SEG_ENC)    ? enc_rvalid    :
		(r_seg == SEG_EXT)    ? ext_rvalid    : ram_rvalid;
	assign dbus_rdata  =
		(r_seg == SEG_PERIPH) ? periph_rdata  :
		(r_seg == SEG_ENC)    ? enc_rdata     :
		(r_seg == SEG_EXT)    ? ext_rdata     : ram_rdata;
	assign dbus_rresp  =
		(r_seg == SEG_PERIPH) ? periph_rresp  :
		(r_seg == SEG_ENC)    ? enc_rresp     :
		(r_seg == SEG_EXT)    ? ext_rresp     : ram_rresp;
	...
	assign ext_rready    = (r_seg == SEG_EXT)    && dbus_rready;
