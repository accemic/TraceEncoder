...
# rocket2/cva6_linux); a rename avoids the generic `duo_`-prefixed name
# implying ownership it does not have, and avoids a hard build dependency on
# a directory owned by a concurrent, uncoordinated migration worker. Content
# is otherwise byte-identical (already English, already SPDX-headed in the
# source repo).
