...
# Sourced by run_rocket_bitstream.tcl inside the open Vivado project context;
# each IP is created as a plain XCI with an explicit configuration and
# instantiated from rocket_kv260_top.sv.
#
...
# AXI4 -> AXI4-Lite protocol converter -> rocket_soc_top @ 0xA000_0000.
#
...
# This is the vivado/kv260_app-family
# copy (S_AXI_GP2/GP3 enabled below), the same shared file already vendored
# byte-faithfully by examples/kv260/mbv/fpga/gen_ip.tcl for the mbv example --
# S_AXI_GP2 (DDR trace sink) and S_AXI_GP3 (Rocket memory path) are both
# actually WIRED in this example's own top (rocket_kv260_top.sv), unlike
# mbv's, which ties both off -- kept byte-faithful to the flow that built this
# example's bitstream in the predecessor repository rather than silently trimmed during
# migration. Vendored per-example (own copy here and in ../../rocket2/fpga/),
# same reasoning as mbv's own copy: no cross-example TCL dependency for a
# small, self-contained generation script.
...
# (ct_soc_ddr_sink AXI master) -- write-only, see rocket_kv260_top.sv.
# S_AXI_GP3 = S_AXI_HP1_FPD: 64-bit slave port for the Rocket memory path
# (rocket_mem_window -> mem_axi_*) -> PS DDR window 0x6400_0000 (board
# memory map).
...
# at that, the constant-high tie in rocket_kv260_top.sv would hold everything
# in permanent reset and every PS access would hang).
