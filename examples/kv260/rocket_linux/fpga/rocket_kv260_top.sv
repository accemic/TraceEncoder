...
/**
 * @brief    Bitstream top of the Rocket RV64 demonstrator (RV64 program, R4a).
 *
 * @details
 ...
 *         -> rocket_soc_top @ 0xA000_0000 (4 MiB aperture)
 ...
 *   ctrace_resmem.dtso -- taken over unchanged):
 ...
 *
 *   The Rocket is the only core, so it gets the full encoder provisioning:
 *   DUAL (N-Trace + E-Trace, runtime-selectable via trTeProtocolSel). No
 *   funnel -- the encoder's ATB goes directly into the ring.
 */
module rocket_kv260_top #(
	...
	// from run_rocket_bitstream.tcl (first tclarg) -- before package W1 this
	// tclarg was read but never wired up anywhere (B-W1-2).
	parameter bit EN_ETRACE = 1'b1
) (
	...
	rocket_soc_top #(
		...
		.EN_ETRACE   (EN_ETRACE)        // 1 = sole core, full DUAL provisioning
	) soc (
		.clk            (pl_clk0),
		.resetn         (aresetn),
