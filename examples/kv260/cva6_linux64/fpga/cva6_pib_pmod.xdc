...
# (2026-08-17). Vendored under a
# renamed, non-`duo_`-prefixed filename rather than cross-referenced into the
# duo example's (concurrently, independently migrated) directory -- this
# constraint file only pins physical PMOD pads, identical across every
# kv260_app-family top that exposes a PIB port (duo/trio/rocket_linux/
# rocket2/cva6_linux/cva6_linux64/cva6_2); a rename avoids the generic
# `duo_`-prefixed name implying ownership it does not have, and avoids a hard
# build dependency on a directory owned by a concurrent, uncoordinated
# migration worker. Content is otherwise byte-identical (already English,
# already SPDX-headed in the source repo) -- exactly the precedent
# rocket_linux/rocket2 already set with their own rocket_pib_pmod.xdc copies.
