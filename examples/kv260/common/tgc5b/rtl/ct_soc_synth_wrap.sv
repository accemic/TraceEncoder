...
/**
 * @brief    Example SoC: MINRES TGC5B RISC-V core + CEDARtools.TraceEncoder N-Trace encoder
 *           (simulation DUT and OOC synthesis target; the KV260 app wraps it
 *           behind ct_soc_top).
 *
 * @details
 *   A minimal but complete system showing how to drive `ct_encoder` from a real
 *   core. Pieces (all in examples/kv260/common/tgc5b/):
 *
 *     TGC5B_AXI4L_H2E  the RV32I core (vendored under cpu/)
 *     ct_tip_adapter   maps the core H2E trace port onto the encoder TIP
 *     ct_encoder       the DUT, SPLIT_DATA_ACCESS=1 to match the core data port
 *     ct_soc_ram       dual-port program/data RAM (iBus read + dBus data)
 *     ct_soc_periph    CLINT + INTC (RDL-generated regs), drives the 3 core IRQs
 *     ct_axil_to_wb    bridges the encoder Wishbone CSRs onto the core dBus
 *
 *   Data-bus (dBus, AXI4-Lite) memory map — see rdl/ct_soc.rdl:
 *     0x0000_0000  RAM      program + data
 *     0x1000_0000  PERIPH   CLINT + INTC
 *     0x2000_0000  ENCODER  CEDARtools.TraceEncoder CSRs (via Wishbone bridge)
 *
 *   This is a *pin-reduced synthesis top* (flat ports, no interfaces) usable as
 *   an out-of-context utilization/timing target, and as the DUT of ct_soc_tb.
 *   The single clock feeds all five encoder domains; a real integration may
 *   separate them. The core's uncompressed golden trace port (core_trace_pc /
 *   core_trace_valid) is brought out as the verification reference.
 */

module ct_soc_synth_wrap #(
	...
	ct_encoder #(.CORE_XLEN(32), .SPLIT_DATA_ACCESS(1)) ct_encoder_inst (
		.tip_clk      (clk),
		.tip_rst      (rst),
		.tip          (tip.slave),
		.wb_clk       (clk),
