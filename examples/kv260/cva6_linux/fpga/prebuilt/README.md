...
* `cva6_linux_ctrace_kv260/` -- `.bit.bin` (bootgen, fpga-manager format), device-tree overlay, `shell.json`, and a `MANIFEST.sha256` that verifies the set.
...
bash examples/kv260/common/board/deploy_kv260_app.sh \
     --app-dir examples/kv260/cva6_linux/fpga/prebuilt/cva6_linux_ctrace_kv260 \
...
```bash
cd examples/kv260/cva6_linux/fpga/prebuilt/cva6_linux_ctrace_kv260 && sha256sum -c MANIFEST.sha256
```
