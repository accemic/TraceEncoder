...
# Sourced by run_cva6_linux_bitstream.tcl inside the open Vivado project
# context; each IP is created as a plain XCI with an explicit configuration
# and instantiated from cva6_linux_kv260_top.sv.
#
...
# AXI4 -> AXI4-Lite protocol converter -> cva6_linux_soc_top @ 0xA000_0000.
#
...
# S_AXI_GP2 (DDR trace sink) and S_AXI_GP3 (CVA6 memory path) are both
# actually WIRED in this example's own top (cva6_linux_kv260_top.sv), unlike
# mbv's, which ties both off. It is therefore a superset of
# single-core case -- kept byte-faithful to the flow that built this
# example's bitstream in the predecessor repository rather than silently trimmed during
# migration. Vendored per-example (own copy here, in ../../cva6_linux64/fpga/
# and in ../../cva6_2/fpga/), same reasoning as mbv's own copy: no
# cross-example TCL dependency for a small, self-contained generation
# script -- this is now the FOURTH vendored copy in the tree (after mbv,
# rocket_linux, rocket2), promotion to tools/ is increasingly overdue but
# out of this migration's write scope.
...
# (ct_soc_ddr_sink AXI master) -- write-only, see cva6_linux_kv260_top.sv.
