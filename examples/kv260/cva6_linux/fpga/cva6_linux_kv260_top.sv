...
/**
 * @brief    Bitstream top of the Linux CVA6 demonstrator: Zynq PS + AXI
 *           plumbing around cva6_linux_soc_top.
 *
 * @details
 *   Same PS provisioning as the trio/duo example tops (the same four XCIs
 *   from gen_ip.tcl, the same 75 MHz domain) -- only the SoC instance
 *   differs. Three PS interfaces:
 *
 *     M_AXI_HPM0_FPD (128 bit) -> dwc 128->32 -> pc AXI4->Lite
 *         -> cva6_linux_soc_top @ 0xA000_0000 (4 MiB aperture)
 ...
 *   through 1:1 (no translation in the PL).
 *
 *   The CVA6 is the ONLY core here, so it gets the full encoder
 *   provisioning: DUAL (N-Trace + E-Trace, runtime choice via
 *   trTeProtocolSel). No funnel -- the encoder's ATB goes directly into
 *   the ring.
 */
module cva6_linux_kv260_top (
	...
	cva6_linux_soc_top #(
		.BOOT_ADDR   (32'h6400_0000),
		.CLK_HZ      (75_000_000),
		.TICK_HZ     (1_000_000),
		.CON_BYTES   (65536),
		.TRACE_DEPTH (262144),     // 1 MiB URAM ring (same as the trio example)
		.EN_ETRACE   (1'b0)        // N-Trace (dual retired 2026-08; board verdict decodes -deco)
	) soc (
		.clk            (pl_clk0),
		.resetn         (aresetn),
...
endmodule // cva6_linux_kv260_top
