...
# Sourced by create_project_kv260.tcl inside the open Vivado project context;
# each IP is created as a plain XCI with an explicit configuration and
# instantiated from trio_kv260_top.sv.
#
...
# widths mis-steer write byte lanes, see trio_kv260_top.sv) -> AXI data
# width converter 128->32 -> AXI4 -> AXI4-Lite protocol converter ->
# trio_soc_top @ 0xA000_0000.
#
...
# This is the vivado/kv260_app-
# family copy (S_AXI_GP2/GP3 enabled below for the DDR trace sink / CVA6
# memory path used by the sibling kv260_app tops -- duo/trio -- that share
# this Zynq PS IP configuration; the single-core mbv_kv260_top.sv ties those
# inputs off). BYTE-IDENTICAL to ../../mbv/fpga/gen_ip.tcl and
# ../../duo/fpga/gen_ip.tcl except for this header's top-name references --
# all three examples in the kv260_app family need the exact same PS IP
# configuration, so this is intentionally not de-duplicated into a single
# shared copy in this migration step (see the migration notes' disclosure).
...
# S_AXI_GP2 = S_AXI_HP0_FPD: 32-bit slave port for the DDR4 trace sink
# (ct_soc_ddr_sink AXI master, duo_kv260_top). In the single-core top
# (mbv_kv260_top, examples/kv260/mbv/) the saxigp2 inputs are explicitly tied off.
# S_AXI_GP3 = S_AXI_HP1_FPD: 64-bit slave port for the CVA6 memory path
# (trio_kv260_top) -> DDR4 window 0x7C00_0000 (board memory map).
# In the mbv/duo tops the saxigp3 inputs are tied off.
...
# at that, the constant-high tie in trio_kv260_top.sv holds everything in
