...
// the predecessor repository gate C5 (trio): a copy of duo_kv260_top plus a third core
// branch -- SoC instance = trio_soc_top (MBV + TGC5B + CVA6), additionally
// S_AXI_HP1_FPD (saxigp3, 64 bit) as the CVA6 memory path into the PS DDR4
// window 0x6400_0000 +192 MiB (address plan v4; reserved-memory via
// ../../common/board/ctrace_resmem.dtso, which covers 0x5000_0000 +512 MiB).
// Corrected 2026-08-19: this comment named 0x7C00_0000, an address from the
// plan before v4 that lies OUTSIDE the reservation -- see the note in
// ../README.md. The authoritative split is trio_soc_top.sv's DDR_BASE entry.
// The ct_soc_kv260_ps IP needs the regen with
// S_AXI_GP3 (gen_ip.tcl).
module trio_kv260_top (
	...
	// -- DDR4-Sink: AXI4-Master des SoC -> PS S_AXI_HP0_FPD (saxigp2) --------
	...
	// -- CVA6 memory path: AXI4 master of the SoC -> PS S_AXI_HP1_FPD (saxigp3)
	...
	uwire logic [5:0]  hp1_bid_w, hp1_rid_w;   // PS side is 6-bit ID
	...
	ct_soc_kv260_ps ps (
		...
		.saxigp2_rvalid    (),
		.saxigp2_rready    (1'b1),
		// S_AXI_HP1_FPD (saxigp3, 64 bit) <- CVA6 memory path (R/W). awatop:
		// the HP port has no atop input; CVA6 AMOs are off by config (RVA
		// unused in software) and axi_ram_sim/assertions enforce atop==0 in
		// simulation.
	...
	// Backend fit for the BOARD build (2026-07-25, measured): all THREE
	// encoders as DUAL do NOT fit the xck26 -- synthesis 120,665 LUT =
	// 103.03 %, the placer aborts (13,224 of 12,757 CLBs). So here: CVA6
	// (ENC2) as DUAL -- the core with the native E-Trace ingress -- and
	// MBV/TGC5B as N-Trace-only. The funnel mixes both framings into ONE
	// stream (the te_raw tag applies only to the CVA6 share); the full
	// triple-DUAL configuration is verified in simulation.
	trio_soc_top #(
		.ENC0_ETRACE (1'b0),
		.ENC1_ETRACE (1'b0),
		.ENC2_ETRACE (1'b1)
		...
		.pib_clk       (pib_clk),
		.pib_data      (pib_data),
		.m2_axi_awid   (hp1_awid),
		.m2_axi_awaddr (hp1_awaddr),
		.m2_axi_awlen  (hp1_awlen),
		.m2_axi_awsize (hp1_awsize),
		.m2_axi_awburst(hp1_awburst),
		.m2_axi_awlock (hp1_awlock),
		.m2_axi_awcache(hp1_awcache),
		.m2_axi_awprot (hp1_awprot),
		.m2_axi_awatop (hp1_awatop),   // must stay 0 (no atop on the HP port)
		.m2_axi_awvalid(hp1_awvalid),
		.m2_axi_awready(hp1_awready),
		.m2_axi_wdata  (hp1_wdata),
		.m2_axi_wstrb  (hp1_wstrb),
		.m2_axi_wlast  (hp1_wlast),
		.m2_axi_wvalid (hp1_wvalid),
		.m2_axi_wready (hp1_wready),
		.m2_axi_bid    (hp1_bid),
		.m2_axi_bresp  (hp1_bresp),
		.m2_axi_bvalid (hp1_bvalid),
		.m2_axi_bready (hp1_bready),
		.m2_axi_arid   (hp1_arid),
		.m2_axi_araddr (hp1_araddr),
		.m2_axi_arlen  (hp1_arlen),
		.m2_axi_arsize (hp1_arsize),
		.m2_axi_arburst(hp1_arburst),
		.m2_axi_arlock (hp1_arlock),
		.m2_axi_arcache(hp1_arcache),
		.m2_axi_arprot (hp1_arprot),
		.m2_axi_arvalid(hp1_arvalid),
		.m2_axi_arready(hp1_arready),
		.m2_axi_rid    (hp1_rid),
		.m2_axi_rdata  (hp1_rdata),
		.m2_axi_rresp  (hp1_rresp),
		.m2_axi_rlast  (hp1_rlast),
		.m2_axi_rvalid (hp1_rvalid),
		.m2_axi_rready (hp1_rready),
