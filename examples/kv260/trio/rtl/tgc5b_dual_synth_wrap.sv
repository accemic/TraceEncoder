...
/**
 * @brief    TGC5B + CTTE wrapper with the DUAL (N-Trace + E-Trace)
 *           encoder backend (local copy of examples/kv260/common/tgc5b/rtl/
 *           ct_soc_synth_wrap.sv, for the trio example's dual-protocol
 *           funnel).
 *
 * @details
 *   1:1 copy of `../../common/tgc5b/rtl/ct_soc_synth_wrap.sv` (this
 *   repository's own reference example, read-only) with exactly TWO
 *   documented deviations, both needed by trio_soc_top.sv's dual-protocol
 *   funnel (see that file's header, "DUAL PROTOCOL"):
 *
 *   (a) NEW parameter `EN_ETRACE` (default `ct_pkg::CT_EN_ETRACE`) and NEW
 *       port `atb_te_raw` [1 bit], wired straight through to `ct_encoder`'s
 *       own `EN_ETRACE` parameter and `atb_te_raw` output. This exposes
 *       which framing (N-Trace/Nexus or E-Trace raw bytes) the encoder is
 *       actually driving on ATB, so the funnel can read it as a signal
 *       instead of tracking a second, software-programmed copy.
 *   (b) Module renamed `ct_soc_synth_wrap` -> `tgc5b_dual_synth_wrap` (both
 *       the reference wrapper and this copy are compiled into the same
 *       trio build; a name collision is not acceptable, see this
 *       migration's disclosure below).
 *
 *   DISCLOSURE: this wrapper does not exist upstream. The vendored
 *   `ct_soc_synth_wrap.sv` of the predecessor repository ALREADY carried
 *   deviation (a) baked directly into the `ct_soc_synth_wrap` module itself
 *   (no rename), because there that name was unique; its trio top
 *   instantiated `ct_soc_synth_wrap` directly for the TGC5B branch with
 *   `.EN_ETRACE(...)`/`.atb_te_raw(...)` connected directly. This
 *   repository's `examples/kv260/common/tgc5b/rtl/ct_soc_synth_wrap.sv` is a
 *   DIFFERENT, divergent revision that never gained that extension (a
 *   real, pre-existing difference between the two repositories' copies of
 *   this shared reference module, found during this migration -- confirmed
 *   by a full `diff` against that vendored copy: the only
 *   substantive delta is exactly deviations (a) above, everything else
 *   matches). Modifying `examples/kv260/common/tgc5b/rtl/ct_soc_synth_wrap.sv`
 *   itself is out of this migration's write scope (and would affect every
 *   other consumer of that shared file, including `duo`, which does NOT
 *   need dual-protocol support for its TGC5B branch and must keep using
 *   the unmodified original). This file is therefore the trio-only,
 *   separately named carrier of that extension, following the exact same
 *   "local copy with documented deviations" pattern already established by
 *   `examples/kv260/tgc5b2_axis_wp/rtl/tgc5b_wp_synth_wrap.sv` for a
 *   different deviation set.
 *
 *   Everything else (core, TIP adapter, RAM + loader mux, CLINT/INTC, dBus
 *   decode, external Wishbone config port) is unchanged from the
 *   reference -- see that file's header for the memory map and the
 *   core_rst_hold / cfg_wb_en contracts.
 */

module tgc5b_dual_synth_wrap #(
	...
	// CORE_XLEN is mandatory on the gold-standard encoder: ct_encoder carries two
	// guards, and genCoreXlenMismatch accepts exactly ct_pkg::CT_XLEN. The TGC5B is
	// a 32-bit core, so 32 it is -- without this the example dies at elaboration.
	ct_encoder #(.CORE_XLEN(32), .SPLIT_DATA_ACCESS(1), .EN_ETRACE(EN_ETRACE), .EN_NTRACE(!EN_ETRACE)) ct_encoder_inst (
		.tip_clk      (clk),
		.tip_rst      (rst),
		.tip          (tip.slave),
		.wb_clk       (clk),
	...
	// AXIS instrumentation: always-ready sink. NOTE: the encoder qualifies its
	// elements with axis.tstrb and never asserts axis.tlast, so the tkeep/tlast
	// lines exported here are undriven and read 0 -- a consumer identifies the
	// valid elements from the command in tid (see ct_soc_axis_buf).
